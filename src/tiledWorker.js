...
class Map{
  ...
  getLayer(z,x,y,w,h){
    try{return this.map.layers[z].draw().getImageData(x||0,y||0,w||this.map.width,h||this.map.height)}catch{return Canvas.createImageData(this.map.width,this.map.height)}
  }
  setTile(z,x,y,id){
...
parentPort.on("message", (msg) => {
  parentPort.postMessage({ id: msg.id, res: map[msg.type](msg) });
});